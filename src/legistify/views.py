...
def send_request(request,lid):
	if request.session['type']:
		if request.session['type']=='user':
			try:
				ContactLawyer.objects.create(uid=request.user, lid=lid, status=1).save()
				context = {
							"error" : True,
							"error_message":"Send the request to lawyer"
				}
...
def accept_request(request,uid):
	if request.session['type']:
		if request.session['type']=='user':
			return render(request, "UserDashboard.html", {})
		elif request.session['type']=='lawyer':
			try:
				ContactLawyer.objects.get(uid=uid, lid=request.user).update(status=2).save()
				context = {
							"error" : True,
							"error_message":"Accepted the request to lawyer"
				}
...
def reject_request(request,uid):
	if request.session['type']:
		if request.session['type']=='user':
			return render(request, "UserDashboard.html", {})
		elif request.session['type']=='lawyer':
			try:
				ContactLawyer.objects.get(uid=uid, lid=request.user).update(status=3).save()
				context = {
							"error" : True,
							"error_message":"Accepted the request to lawyer"
				}
